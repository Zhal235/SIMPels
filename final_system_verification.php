...
use Illuminate\    // Simulate API c    
    $tagihan_insidentil = DB::table('tagihan_santris as ts')
        ...
        ->get();$tagihan_rutin = DB::table('tagihan_santris as ts')
        ->leftJoin('jenis_tagihans as jt', 'ts.jenis_tagihan_id', '=', 'jt.id')
        ->where('ts.santri_id', $first_santri->id)
        ->where('ts.status', 'aktif')
        ->where(function($query) {
            $query->where('jt.kategori_tagihan', 'Rutin')
                  ->orWhere('ts.is_bulanan', true);
        })
        ->select('ts.*', 'jt.nama', 'jt.kategori_tagihan')
        ->get();ades\DB;
...
$kategori_stats = DB::table('tagihan_santris as ts')
    ->leftJoin('jenis_tagihan as jt', 'ts.jenis_tagihan_id', '=', 'jt.id')
    ->select(
        DB::raw('jt.kategori_tagihan'),
        DB::raw('ts.is_bulanan'),
        DB::raw('COUNT(*) as count')
    )
    ->groupBy('jt.kategori_tagihan', 'ts.is_bulanan')
    ->get();

echo "\nTagihan distribution by category:\n";
foreach ($kategori_stats as $stat) {
    $category = $stat->kategori_tagihan ?: 'NULL';
    $bulanan = $stat->is_bulanan ? 'Bulanan' : 'Non-Bulanan';
    echo "  - $category ($bulanan): {$stat->count} records\n";
}
...
$first_santri = DB::table('santri')->first();
if ($first_santri) {
    echo "Testing with santri: {$first_santri->nama_lengkap} (ID: {$first_santri->id})\n";
      // Simulate API call
    $tagihan_rutin = DB::table('tagihan_santris as ts')
        ->leftJoin('jenis_tagihan as jt', 'ts.jenis_tagihan_id', '=', 'jt.id')
        ->where('ts.santri_id', $first_santri->id)
        ->where('ts.status', 'Belum Bayar')
        ->where(function($query) {
            $query->where('jt.kategori_tagihan', 'Rutin')
                  ->orWhere('ts.is_bulanan', true);
        })
    ...
    $tagihan_insidentil = DB::table('tagihan_santris as ts')
        ...
        ->where('ts.is_bulanan', false)
        ->select('ts.*', 'jt.nama_tagihan', 'jt.kategori_tagihan')
        ->get();
    
    echo "Tagihan Rutin found: " . count($tagihan_rutin) . "\n";
    echo "Tagihan Insidentil found: " . count($tagihan_insidentil) . "\n";
    
    if (count($tagihan_rutin) > 0) {
        echo "\nSample Rutin tagihan:\n";
        foreach (array_slice($tagihan_rutin->toArray(), 0, 3) as $tagihan) {
            echo "  - {$tagihan->nama_tagihan} | {$tagihan->bulan} | Rp " . number_format($tagihan->jumlah_tagihan) . "\n";
        }
    }
...
foreach ($months_data as $row) {
    ...
    if (!isset($month_summary[$month])) {
        $month_summary[$month] = ['Rutin' => 0, 'Insidentil' => 0, 'Unknown' => 0];
    }
...
foreach (array_slice($month_summary, 0, 5, true) as $month => $data) {
    echo "  $month: Rutin={$data['Rutin']}, Insidentil={$data['Insidentil']}, Unknown={$data['Unknown']}\n";
}
...
if ($first_santri) {
    ...
    if ($sample_tagihan) {
        ...
        echo "  - Nama Tagihan: {$sample_tagihan->nama_tagihan}\n";
        echo "  - Kategori: " . ($sample_tagihan->kategori_tagihan ?: 'NULL') . "\n";
        echo "  - Is Bulanan: " . ($sample_tagihan->is_bulanan ? 'true' : 'false') . "\n";
        echo "  - Bulan: {$sample_tagihan->bulan}\n";
        echo "  - Jumlah: Rp " . number_format($sample_tagihan->jumlah_tagihan) . "\n";
        echo "  - Status: {$sample_tagihan->status}\n";
        
        // Test categorization logic
        $is_rutin = ($sample_tagihan->kategori_tagihan == 'Rutin' || $sample_tagihan->is_bulanan);
        $is_insidentil = ($sample_tagihan->kategori_tagihan == 'Insidentil' && !$sample_tagihan->is_bulanan);
        
        echo "  - Categorized as Rutin: " . ($is_rutin ? 'YES' : 'NO') . "\n";
        echo "  - Categorized as Insidentil: " . ($is_insidentil ? 'YES' : 'NO') . "\n";
    }
}
