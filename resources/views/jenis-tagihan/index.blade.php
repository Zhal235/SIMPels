                    ...
                    @endif                    <table class="w-full whitespace-no-wrap">
                        <thead>
                            <tr class="text-xs font-semibold tracking-wide text-left text-gray-500 uppercase border-b dark:border-gray-700 bg-gray-50 dark:text-gray-400 dark:bg-gray-800">
                                ...
                                <th class="px-4 py-3">Nominal per Kelas</th>
                            ...
                            <tr class="text-gray-700 dark:text-gray-400">
                                <td class="px-4 py-3">
                                    <div class="flex items-center text-sm">
                                        <div>
                                            <p class="font-semibold">{{ $tagihan->nama }}</p>
                                            <p class="text-xs text-gray-600 dark:text-gray-400">{{ $tagihan->deskripsi }}</p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-3 text-sm">
                                    <span class="px-2 py-1 font-semibold leading-tight rounded-full {{ $tagihan->kategori_tagihan == 'Rutin' ? 'text-green-700 bg-green-100' : 'text-orange-700 bg-orange-100' }}">
                                        {{ $tagihan->kategori_tagihan }}
                                    </span>
                                </td>                                <td class="px-4 py-3 text-sm">
                                    <span class="px-2 py-1 font-semibold leading-tight rounded-full {{ $tagihan->is_bulanan ? 'text-green-700 bg-green-100' : 'text-purple-700 bg-purple-100' }}">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center space-x-4 text-sm">                                        <a href="{{ route('keuangan.jenis-tagihan.edit', $tagihan->id) }}" 
                                        ...
                                        <form action="{{ route('keuangan.jenis-tagihan.destroy', $tagihan->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
                                                    class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                                    aria-label="Delete"
                                                    onclick="return confirm('Apakah Anda yakin ingin menghapus jenis tagihan ini?')">
                                                <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
